...
// this is a bit overkill but I decided to go full OO on everything
class Utils {
    /**
     ...
     ** @return a character array (heap allocated)
     **/
    static icu::UnicodeString char_to_unicode(const char *cc) {
     ...
     **/
    static UChar *unicode_to_uchar(const icu::UnicodeString &str) {
     ...
     **/
    static icu::UnicodeString unicode_convert_uint(unsigned int n) {
        std::stringstream ss;
        ss << n;
     ...
     **/
    static icu::UnicodeString unicode_concat(const icu::UnicodeString &s0,
     ...
     ** @return the unescaped string
     **/
    static bool unicode_endswith(const icu::UnicodeString &s,
     ...
     **/
    static int64_t convert_to_int(const icu::UnicodeString &s) {
     ...
     ** @return the integer recognized
     **/
    static int64_t convert_to_hexint(const icu::UnicodeString &s) {
        int i = 0;
        int64_t n = 0;
        UChar32 c = s.char32At(i);
        while (c != 0xffff) {
     ...
     ** @return the integer recognized
     **/
    static double convert_to_float(const icu::UnicodeString &s) {
        char *buf = unicode_to_char(s);
     ...
     **/
    static UChar32 convert_to_char(const icu::UnicodeString &s) {
     ...
     ** @return the integer recognized
     **/
    static icu::UnicodeString convert_from_int(const int64_t &n) {
        std::stringstream ss;
        ss << n;
        icu::UnicodeString u(ss.str().c_str());
        return u;
     ...
     ** @return the integer recognized
     **/
    static icu::UnicodeString convert_from_float(const double &f) {
        std::stringstream ss;
        ss << f;
        icu::UnicodeString u(ss.str().c_str());
        return u;
     ...
     ** @return the char recognized
     **/
    static icu::UnicodeString convert_from_char(const UChar32 &c) {
        std::stringstream ss;
        ss << c;
        icu::UnicodeString u(ss.str().c_str());
     ...
     ** @return the integer recognized
     **/
    static icu::UnicodeString convert_to_text(const icu::UnicodeString &s) {
        auto s0 = unicode_strip_quotes(s);
        auto s1 = unicode_unescape(s0);
        return s1;
    }
    ...
    static UChar *read_utf8_file(const char *filename);
    static void write_utf8_file(const char *filename, UChar *str);
    /**
     ...
     **/
    static icu::UnicodeString file_read(const icu::UnicodeString &filename) {
     ...
     ** @param str          the string
     **/
    static void file_write(const icu::UnicodeString &filename,
