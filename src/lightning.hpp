...
inline void vm_object_ptr_construct_n(VMObjectPtr* p, int n) {
    ...
    for (int i = 0; i < n; i++) {
        //    TRACE_JIT(std::cerr << "construct_n " << p << " " << i <<
        //    std::endl);
        new (&(p[i])) VMObjectPtr();
    }
};
...
inline void load(VM* vm, VMObjectPtr* p, VMObjectPtr* x) {
    TRACE_JIT(std::cerr << "LOAD " << vm << ", " << p << ", " << x
                        << std::endl);
    p[0] = *x;
    ;
};
...
inline void debug(VM* vm, VMObjectPtr* a, int n, int* flag) {
    ...
    std::cerr << "flag(" << *flag << ", " << (((bool)*flag) ? "true" : "false")
              << ")" << std::endl;
    std::cerr << "############## DEBUG END ########################"
              << std::endl;
};
...
// DEBUG
inline void debug_registers(void* v0, void* v1, void* v2, void* r0, void* r1,
                            void* r2, void* fp) {
    std::cerr << "############## REGISTERS ########################"
              << std::endl;
...
inline void op_array(VM* vm, VMObjectPtr* a, int x, int y, int z) {
    TRACE_JIT(std::cerr << "OP_ARRAY r" << x << ", r" << y << ", r" << z
                        << std::endl);
    //  if (y <= z) {
    auto n = (z - y) + 1;
...
inline void op_set(VM* vm, VMObjectPtr* a, int x, int y, int z) {
    TRACE_JIT(std::cerr << "OP_MOV r" << x << ", r" << y << ", r" << z
                        << std::endl);
...
inline void op_takex(VM* vm, VMObjectPtr* a, int x, int y, int z, int i,
                     void** flag) {
    TRACE_JIT(std::cerr << "OP_TAKEX r" << x << ", r" << y << ", r" << z
                        << ", i" << i << std::endl);
    int n = (y - x) + 1;
...
inline void op_split(VM* vm, VMObjectPtr* a, int x, int y, int z, void** flag) {
    TRACE_JIT(std::cerr << "OP_SPLIT r" << x << ", r" << y << ", r" << z
                        << std::endl);
    auto n = (y - x) + 1;
    auto a0 = VMObjectArray::cast(a[z]);
    if (((int)a0->size()) != n) {
...
inline void op_concatx(VM* vm, VMObjectPtr* a, int x, int y, int z, int n) {
    TRACE_JIT(std::cerr << "OP_CONCATX r" << x << ", r" << y << ", r" << z
                        << ", i" << n << std::endl);
...
class BytecodePass {
public:
    BytecodePass(VM* m, const Code& c) : _code(c), _pc(0), _machine(m) {
    }
    ...
    virtual void op_concatx(uint32_t pc, reg_t x, reg_t y, reg_t z,
                            uint16_t i) {
    }
...
class DataPass {
public:
    DataPass(VM* m, const Data& d) : _data(d), _machine(m) {
    }
...
public:
    ...
    virtual void op_takex(uint32_t pc, reg_t x, reg_t y, reg_t z,
                          uint16_t i) override {
        max(x);
        max(y);
        max(z);
...
public:
    EmitNative(VM* m, const VMObjectPtr& o)
        : BytecodePass(m, o),
          _proc(nullptr),
          _data(PushData(m, o)),
          _analyzebytecode(m, o) {
    }
    ...
    void emit_label(uint32_t pc) {
        auto ll = _analyzebytecode.labels();
        if (ll.count(pc) > 0) {
            // TRACE_JIT(std::cerr << "emitted label for " << (void*)pc <<
            // std::endl);
    ...
    void* emit() {
        ...
        _flag_offset =
            jit_allocai(sizeof(void*));  // stores a bool but use word size
...
public:
    VMObjectLightning(VM* m, const Code& c, const Data& d, const symbol_t s,
                      void* p)
    ...
    VMObjectPtr reduce(const VMObjectPtr& thunk) const override {
        VMObjectPtr r;
        // VM, thunk, result
        TRACE_JIT(std::cerr << "native call " << to_text() << " on " << _proc
                            << std::endl);
        ...
        TRACE_JIT(std::cerr << "arguments: vm(" << (void*)machine()
                            << ") thunk(" << (void*)&tt << ") return("
                            << (void*)&r << ")" << std::endl);
        f(machine(), &tt, &r);
