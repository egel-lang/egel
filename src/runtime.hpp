...
public:
    ...
    virtual void enter_binding(const symbol_t o, const symbol_t l, const symbol_t r, const symbol_t e) = 0;
    virtual void enter_binding(const symbol_t o, const symbol_t a, const symbol_t e) = 0;
    virtual symbol_t get_binding(const symbol_t o, const symbol_t l, const symbol_t r) = 0;
    virtual symbol_t get_binding(const symbol_t o, const symbol_t a) = 0;
...
class VMObjectPrefix: public VMObjectCombinator {
public:
    VMObjectPrefix(VM* m, const UnicodeString& n0, const UnicodeString& n1): 
         VMObjectCombinator(VM_OBJECT_FLAG_INTERNAL, m, n0, n1) {
    }

    VMObjectPrefix(VM* m, const std::vector<UnicodeString>& nn, const UnicodeString& n)
        : VMObjectCombinator(VM_OBJECT_FLAG_INTERNAL, m, nn, n) {
    };

    VMObjectPrefix(VM* m, const symbol_t s): 
         VMObjectCombinator(VM_OBJECT_FLAG_INTERNAL, m, s) {
    }

    VMObjectPrefix(const VMObjectPrefix& o):
        VMObjectPrefix(o.machine(), o.symbol()) {
    }

    VMObjectPtr clone() const override {
        return VMObjectPtr(new VMObjectPrefix(*this));
    }

    VMObjectPtr reduce(const VMObjectPtr& thunk) const override {
        VMObjectPtrs rr;  // fail goto...
        symbol_t c0, t0, s;
        VMObjectPtr arg0, r;

        auto tt  = VM_OBJECT_ARRAY_VALUE(thunk);

        if (tt.size() <= 5) goto fail;

        arg0 = tt[5];

        c0 = symbol();
        t0 = arg0->symbol();
        s = machine()->get_binding(c0, t0);

        if (s == (symbol_t) -1) goto fail;

        r = machine()->get_data_symbol(s);
...
class VMObjectInfix: public VMObjectCombinator {
public:
    VMObjectInfix(VM* m, const UnicodeString& n0, const UnicodeString& n1): 
         VMObjectCombinator(VM_OBJECT_FLAG_INTERNAL, m, n0, n1) {
    }

    VMObjectInfix(VM* m, const std::vector<UnicodeString>& nn, const UnicodeString& n)
        : VMObjectCombinator(VM_OBJECT_FLAG_INTERNAL, m, nn, n) {
    };

    VMObjectInfix(VM* m, const symbol_t s): 
         VMObjectCombinator(VM_OBJECT_FLAG_INTERNAL, m, s) {
    }

    VMObjectInfix(const VMObjectInfix& o):
        VMObjectInfix(o.machine(), o.symbol()) {
    }

    VMObjectPtr clone() const override {
        return VMObjectPtr(new VMObjectInfix(*this));
    }

    VMObjectPtr reduce(const VMObjectPtr& thunk) const override {
        VMObjectPtrs rr;  // fail goto...
        symbol_t c0, t0, t1, s;
        VMObjectPtr arg0, arg1, r;

        auto tt  = VM_OBJECT_ARRAY_VALUE(thunk);

        if (tt.size() <= 6) goto fail;
        ...
        t0 = arg0->symbol();
        t1 = arg1->symbol();
        s = machine()->get_binding(c0, t0, t1);
